...
const SalaryComparison = ({ data, isOpen, onClose, onSave }) => {
  const [editedData, setEditedData] = useState('');
  const [isEditing, setIsEditing] = useState(false);

  useEffect(() => {
    if (data) {
      setEditedData(typeof data === 'string' ? data : JSON.stringify(data, null, 2));
    }
  }, [data]);
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
          ...
          <CardContent className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            <div className="space-y-4">
              {isEditing ? (
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Entrez votre analyse salariale..."
                    autoFocus
                  />
                </div>
              ) : (
              ...
              <ActionButtons
                content={editedData}
                filename="analyse-salariale.txt"
